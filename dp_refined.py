...
class Network():
    ...
    def define_model(self):
        ...
        print(self.tf_train_samples.shape)
        logits = model(self.tf_train_samples)
        with tf.name_scope('loss'):
            print(logits.shape, self.tf_train_labels.shape)
    ...
    def train(self, train_samples, train_labels, test_samples, test_labels, *, data_iterator_train, data_iterator_test, iteration_steps):
        self.writer = tf.summary.FileWriter('./board', tf.get_default_graph())
        with tf.Session(graph=tf.get_default_graph()) as session:
            tf.global_variables_initializer().run()
            ...
            print('Start Training')
            # batch 1000
            for i, samples, labels in data_iterator_train(train_samples, train_labels, iteration_steps=iteration_steps,
                                                    chunkSize=self.train_batch_size):
                _, l, predictions, summary = session.run(
                    [self.optimizer, self.loss, self.train_prediction, self.merged_train_summary],
                    feed_dict={self.tf_train_samples: samples, self.tf_train_labels: labels}
                ...
                if i % 100 == 0:
                    ...
                    accuracies_test = []
                    for i_test, samples_test, labels_test in data_iterator_test(test_samples, test_labels,
                                                                                chunkSize=self.test_batch_size):
                        result = session.run(
                            self.test_prediction,
                            feed_dict={self.tf_test_samples: samples_test}
                        )
                        # self.writer.add_summary(summary, i)
                        accuracy, cm = self.accuracy(result, labels_test, need_confusion_matrix=True)
                        accuracies_test.append(accuracy)
                    print('Test  Accuracy:', np.average(accuracies_test))
            ###
    ...
    def single_predict(self, data):
        ...
        with tf.Session(graph=tf.get_default_graph()) as session:
            ...
            result = session.run(
                ...
                feed_dict={self.single_input: data}
            )
        print(result)
        return result[0]
